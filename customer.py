...
class CustomNetwork:
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                if inputs is None:
                    continue
                inputs = inputs.to(self.device)  # 迁移输入到 GPU
                labels = labels.to(self.device)  # 迁移标签到 GPU
            ...
            with torch.no_grad():
                for images, labels in val_loader:
                    inputs = images.to(self.device)  # 迁移输入到 GPU
                    labels = labels.to(self.device)  # 迁移标签到 GPU
                    outputs = self.classifier(inputs)
                    loss = criterion(outputs, labels)
                    val_loss += loss.item()
    ...
    def predict(self):
        ...
        for img_name in image_names:
            ...
            if inputs is None:
                continue
            inputs = inputs.to(self.device)  # 迁移输入到 GPU/MPS
